...
{
    public function getUrlCheckoutOrdemServico(Request $request, $id)
    {
        if(!$request->get('taxa')){
            abort(400, 'É preciso informar se haverá taxa ou não na cobrança.');
        }
        ...
        try {
            return response()->json($pagamento->getWhatsappLinkPagamento($preference['response']['init_point'], true), 200);
        } catch (\Exception $e){
            abort(500, 'Erro ao redirecionar: ' . $e->getMessage());
        }
    }

    public function getUrlCheckoutMultiploOrdemServico(Request $request)
    {
        if(!$request->get('taxa')){
        ...
        if ($ordensServicos->isEmpty()) {
            abort(404, 'Nenhuma ordem de serviço encontrada.');
        }
        ...
        try {
            return response()->json(['url' => $pagamento->getWhatsappLinkPagamento($preference['response']['init_point'], true)]);
        } catch (\Exception $e){
            abort(500, 'Erro ao redirecionar: ' . $e->getMessage());
        }
    }
